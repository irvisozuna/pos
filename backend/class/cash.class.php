...
class ControlCash extends CommonObject
{
    ...
    function get_datafromlastclosing()
    {
    	...
    	$sql .=" where type_control = 1 and fk_cash=".$_SESSION['TERMINAL_ID'];
	...
	function getMoneyCash($open=false)
	{
		...
		$sql .=" AND ba.entity =".$conf->entity." AND b.datev=date(now())";
		$sql .=" ORDER BY b.datev ASC , b.datec ASC ";*/
        /* $sql="
          SELECT (
		    (IFNULL((SELECT sum(total_ttc) total_ttc FROM llx_pos_ticket c WHERE date_ticket=date(now()) AND fk_control is null
		                 AND c.fk_cash='".$_SESSION['TERMINAL_ID']."' AND c.entity='".$conf->entity."' AND type='0' 
		         ),0))
		    -(IFNULL((SELECT sum(total_ttc) total_ttc FROM llx_pos_ticket c WHERE date_ticket=date(now()) AND fk_control is null
		                 AND c.fk_cash='".$_SESSION['TERMINAL_ID']."' AND c.entity='".$conf->entity."' AND type='1' 
		         ),0))             		
		     )+(IFNULL((SELECT sum(total_ttc) total_ttc FROM llx_facture a, llx_pos_facture b 
		        WHERE a.rowid=b.fk_facture AND fk_control_cash is null AND datef=date(now()) 
		       AND b.fk_cash='".$_SESSION['TERMINAL_ID']."' AND a.entity='".$conf->entity."' AND b.fk_facture NOT IN (SELECT fk_facture FROM llx_pos_ticket WHERE fk_facture IS NOT NULL)
		       ),0)
		     )amount   
           "; */
  ...
  AND B.fk_type = 'LIQ' WHERE DATE_FORMAT(P.datec, '%Y-%m-%d') = DATE_FORMAT('".$hoy."', '%Y-%m-%d') AND P.note LIKE '% PAGO POR APARTADO%')t1";
          ...
          SELECT (
		    ...
		    -(IFNULL((SELECT sum(total_ttc) total_ttc FROM ".MAIN_DB_PREFIX."pos_ticket c WHERE fk_control is null
		     ...
		     )+(IFNULL((SELECT sum(total_ttc) total_ttc FROM ".MAIN_DB_PREFIX."facture a, ".MAIN_DB_PREFIX."pos_facture b
		        WHERE a.rowid=b.fk_facture AND fk_control_cash is null 
		       AND b.fk_cash='".$_SESSION['TERMINAL_ID']."' AND a.entity='".$conf->entity."' AND b.fk_facture NOT IN (SELECT fk_facture FROM ".MAIN_DB_PREFIX."pos_ticket WHERE fk_facture IS NOT NULL)
		       ),0)
		     )amount
           ";
        }
...
//		$sql.=" where date_closed>'".$date_close."'";
		$sql.=" where fk_statut=1 AND fk_control IS NULL ";
