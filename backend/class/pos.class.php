 ...
 * Copyright (C) 2013 Iv�n Casco              <admin@gestionintegraltn.com>
 *
...
class POS extends CommonObject
{
	...
	public static function getProductsbyCategory($idCat,$more)
	{
		global $db,$conf;
		$ret=array();
		if($idCat) //Productos de la categor�a
		{
	...
	public static function getProductbyId($idProd, $idCust)
	{
		...
		if(!empty( $objp->multiprices[$pricelevel]) && $objp->multiprices[$pricelevel] > 0 ){
			...
			$ret[0]["price"] = round($objp->multiprices[$pricelevel],2);
			$ret[0]["price_ttc"] = round($objp->multiprices_ttc[$pricelevel],2);
			$ret[0]["price_min"] = round($objp->multiprices_min[$pricelevel],2);
			$ret[0]["price_min_ttc"] = round($objp->multiprices_min_ttc[$pricelevel],2);
		}
		else{
			$ret[0]["tva_tx"] = $objp->tva_tx;
			$ret[0]["price_base_type"] = $objp->price_base_type;
			$ret[0]["price"] = $objp->price;
			$ret[0]["price_ttc"] = round($objp->price_ttc,2);
			$ret[0]["price_min"] = round($objp->price_min,2);
			$ret[0]["price_min_ttc"] =round( $objp->price_min_ttc,2);
			if($conf->global->PRODUIT_MULTIPRICES){
				$ret[0]["diff_price"] = 1;
			}
		}
	...
	{
		global $db, $conf;
		//dol_syslog('ALMACEN::'.$_SESSION["uid"]);
		if($warehouse==''){
		$sql="SELECT a.fk_warehouse FROM ".MAIN_DB_PREFIX."pos_cash a, ".MAIN_DB_PREFIX."pos_cashgroup_cash b 
			  WHERE a.rowid=b.fk_cashgroup AND b.fk_user='".$_SESSION["uid"]."'";
		$res=$db->query($sql);
		$rq=$db->fetch_row($res);
		$warehouse=$rq['0'];
		}
		...
		if($mode>=0){
			...
			if(!$stock && $conf->global->POS_STOCK)
			{
				...
				if(!$stock && $conf->global->POS_SERVICES){
					$sql.= " GROUP BY p.rowid, e.label";
					$sql.= " ORDER BY p.label, e.rowid";
				}else{
		...
		dol_syslog('ESTE QUERY BUSCA PROD: '.$sql);
		$resql=$db->query($sql);
		if ($resql)
		{
			$num = $db->num_rows($resql);
        ...
        // Copia de SearchProduct pero especialzada para buscar productos cuando son deboluciones
        public static function SearchProduct2($idSearch,$stock=false, $warehouse,$mode=0)
	{
		global $db, $conf;
	...
	public static function CountProduct($warehouseId)
	{
		...
		dol_syslog('ESTE QUERY CUENTA PRODUCTOS: '.$sql);
		$res=$db->query($sql);
	...
	{	//dol_syslog('LLEGA AQUI GETIMAGE');
		global $conf, $db;
		...
		if($idProd>0)
		{
			...
			//if(DOL_VERSION>='3.7'){
				@$pdir = get_exdir($idProd, 2, 0, 0, $objProd, 'product') . $objProd->ref ."/";
				$dir = $conf->product->multidir_output[$objProd->entity].'/'.$pdir;
				dol_syslog("ESTE DIR::".$dir);
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				if (!$obj->fk_societe)
				{
					...
					{
						$username = $obj->firstname.' '.$obj->lastname;//AMM 1 20140327 cambio u.name por u.lastname
						$internalusers[$i]['code'] = $obj->rowid;
						$internalusers[$i]['label'] = $username;
						
						if ($userstatic->photo) $file=get_exdir($userstatic->id, 2, 0, 0, $userstatic, 'user').getImageFileNameForSize($userstatic->photo, '_mini');
	...
	public static function select_Type_Payments()
	{
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."c_paiement";
		$sql.= " WHERE active > 0 and (id = ".$cash->fk_modepaycash." or id =".$cash->fk_modepaybank." 
				or id IN (SELECT fk_modepay
FROM ".MAIN_DB_PREFIX."pos_terminal_mpagos WHERE fk_terminal=".$cash->id."))";
		$sql.= " ORDER BY id";
		/*MV*/
	...
	{	
		...
		$data = $aryTicket['data'];
		$lines = $data['lines'];
 		dol_syslog('ESTE PAGO: '.$data['customerpay']);
 		dol_syslog('ESTE PAGO2:'.$data['total']);
 		dol_syslog('ESTE PAGO3:'.$data['amount_diff']);
 		dol_syslog('ESTE PAGO4:'.$data['total_ttc']);
 		dol_syslog('ESTE PAGO5:'.$data['type']);
 		dol_syslog('ESTE PAGO6:'.$data['type_control']);
 		dol_syslog('ESTE PAGO7:'.$data['payment_type']);
 		dol_syslog('ESTE PAGO8::'.$data['pay_client_idall']);
 		$datasep=explode("&", $data['pay_client_idall']);
 		dol_syslog('ESTE PAGO9::'.$datasep[0]);
 		dol_syslog('ESTE PAGO10::'.$datasep[1]);
		if(sizeof($data)>0)
		{
			if($data['mode']==0){
				if($data['id'])
				...
				{
					if($data['payment_type']==4 && ($data['customerpay']=='' || $data['customerpay']==0)){
						return ErrorControl(0,'Errormonto');
					}
					global $db;
					$sqla="SELECT pmixtos FROM ".MAIN_DB_PREFIX."pos_cashgroup_cash WHERE fk_user=".$_SESSION['uid'];
					dol_syslog('PERMISOS '.$sqla);
					$resa=$db->query($sqla);
					$resb=$db->fetch_object($resa);
					if($data['payment_type']==4 && $resb->pmixtos==2){
						if($data['customerpay']<$data['total']){
							return ErrorControl(0,'ErrorDiff');
						}
					}
					/* if($data['customerpay']<$data['total'] && $data['payment_type']==4){
						return ErrorControl(0,'ErrorDiff');
					} */
					if($data['payment_type']=='all' && $data['type']!=1){
						dol_syslog('ENtra aqui all');
						$res = self::CreateTicket2($aryTicket);
					}else{
						dol_syslog('ENtra aqui payment');
			...
			else
			{
				...
				if($data['payment_type']=='all' && $data['type']!=1){
					dol_syslog('ENtra aqui all');
					$res = self::CreateFacture2($aryTicket);
				}else{
					dol_syslog('ENtra aqui payment');
...
//			}
		...
		//dol_syslog('TIPO PAGO::'.$data['payment_type']);
		//dol_syslog('CUSTOMER PAY::'.$data['customerpay']);
		$aux2=1;
		 
		 //MV 
		// if($data['payment_type']=='6' && $data['customerpay']!='' && $data['customerpay']>0){
			//$art=4;
			//$data2['payment_type']=$art;
			//$data2 = $data;
			//$data2['payment_type']=$art;
			//$data2['customerpay'] = $data['customerpay'];
			//$aux=$data['total']-$data['customerpay'];
			//$data['customerpay']=$aux;
			//dol_syslog('TIPO PAGO::'.$data['payment_type'].'::');
			//dol_syslog('TIPO PAGO2::'.$data2['payment_type'].'::');
			$data['difpayment']=$data['total']-$data['customerpay'];
			//$data2['difpayment']=$data['difpayment'];
		...
		//}else{
			...
			//}
			dol_syslog('CUSTOMER PAY1::'.$data['customerpay']);
			dol_syslog('CUSTOMER PAY11::'.$object->customer_pay);
			$data['customerpay'] = $object->customer_pay;
		//}
		$object->diff_payment = $data['difpayment'];
		$object->id_source = $data['idsource'];
		if($object->diff_payment<0.05 && $object->diff_payment>0){
				$object->diff_payment=0;
		}
		dol_syslog('CUSTOMER PAY2::'.$object->customer_pay);
		dol_syslog('DIFERENT::'.$object->diff_payment);
		dol_syslog('SOURCE::'.$object->id_source);
		//$object->note=$note;
		...
		else 
		{
			...
			else 
			{
				...
				if($object->statut!=0)
				{
					/* if($data['payment_type']=='6' && $aux2=='2'){
						$data2['ref']=$data['ref'];
						$data2['id']=$data['id'];
						$payment=self::addPayment($data2);
					} */
					//Adding Payments
	...
	Private function CreateTicket2($aryTicket)
	{
		global $db,$user,$conf;
		...
		$i=0;
		$totpay=0;
		dol_syslog("LLEGA A ESTO::".$data['pay_client_idall']);
		$arrpag=explode("&",$data['pay_client_idall']);
		$canarrg=count($arrpag);
		$j=0;
		while($j<$canarrg){
			$arsepa=explode("=",$arrpag[$j]);
			//print "variable:".$d[0].": valor:".$d[1].":<br>";
			$payments = POS::select_Type_Payments();
			foreach($payments as $payment)
			{
				$auxnom='pay_client_id'.$payment['id'];
				dol_syslog("COMPARA ".$arsepa[0]."::".$auxnom);
				if($arsepa[0]==$auxnom && ($arsepa[1]!=0 && $arsepa[1]!=NULL && $arsepa[1]!='')){
						if($i==0){
							dol_syslog('ENTRA PAGO0 '.$payment['id']);
							$data['payment_type']=$payment['id'];
							$data['customerpay'] = $arsepa[1];
							$totpay+=$data['customerpay'];
							//$data['difpayment']=$data['total']-$data['customerpay'];
							$i++;
						}else{
							dol_syslog('ENTRA PAGO'.$i.' '.$payment['id']);
							${$i."data"}=$data;
							${$i."data"}['payment_type']=$payment['id'];
							dol_syslog('PAGO2 TYPE::'.${$i."data"}['customerpay']);
							dol_syslog('PAGO2 TYPE::'.${$i."data"}['payment_type']);
							${$i."data"}['customerpay'] = $arsepa[1];
							$totpay+=${$i."data"}['customerpay'];
							$i++;
						}
				}
			}
		...
		dol_syslog("DATA CUSTOMER PAY::".$totpay);
		$cash = new Cash($db);
		...
		{
			$iaux=$payment['id'];
			if($aryTicket['pay_client_id'.$iaux]!=''){
				if($i==0){
					$data['difpayment']=$data['total']-$totpay;
					$i++;
				}else{
					${$i."data"}['difpayment']=$data['total']-$totpay;
					$i++;
				}
			}
		} */
		...
		while($j<$canarrg){
			...
			foreach($payments as $payment)
			{
				...
				if($arsepa[0]==$auxnom && ($arsepa[1]!=0 && $arsepa[1]!=NULL && $arsepa[1]!='')){
					if($i==0){
						$data['difpayment']=$data['total']-$totpay;
						dol_syslog('PAGO1 DIFF::'.$data['difpayment']);
					$i++;
					}else{
						${$i."data"}['difpayment']=$data['total']-$totpay;
						dol_syslog('PAGO2 DIFF::'.${$i."data"}['difpayment']);
					$i++;
					}
				}
			}
		...
		dol_syslog("ESTE PAYMENT".$data['payment_type']);
		$object = new Ticket($db);
		$object->type=$data['type'];
		$object->socid= $socid;
		$object->statut = $data['state'];
						...
						while($j<$canarrg){
							...
							foreach($payments as $payment)
							{
								...
								if($arsepa[0]==$auxnom && ($arsepa[1]!=0 && $arsepa[1]!=NULL && $arsepa[1]!='')){
									if($i==0){
										$payment=self::addPayment($data);
										dol_syslog('PAGO3.1 DIFF::'.$data['difpayment']);
										$i++;
									}else{
										${$i."data"}['ref']=$data['ref'];
										${$i."data"}['id']=$data['id'];
										dol_syslog('PAGO3.2 DIFF::'.${$i."data"}['difpayment']);
										$payment=self::addPayment(${$i."data"});
										$i++;
									}
								}
							}
	...
	Private function CreateFacture($aryTicket)
	{
        ...
        if($data['mode']==1){
        	dol_syslog("MODE1::".$data['mode']);
			$object = new Facturesim($db);
        }
        else{
        	dol_syslog("MODE2::".$data['mode']);
        	$object = new Facture($db);
        }
		...
		/*
		 ...
		 if($data['payment_type']=='6' && $data['customerpay']!='' && $data['customerpay']>0){
			...
			$aux=$data['total']-$data['customerpay'];
			$data['customerpay']=$aux;
			$data['difpayment']=$data['total']-($data2['customerpay']+$data['customerpay']);
			$data2['difpayment']=$data['difpayment'];
			$object->customer_pay=$data['total'];
			$aux2=2;
		}else{ */
		$data['difpayment']=$data['total']-$data['customerpay'];
			...
			dol_syslog("CUSTOMER PAY::".$object->customer_pay);
			$data['customerpay'] = $object->customer_pay; 
		//$object->customer_pay = $data['customerpay'];
		//}		
		...
		if ($object->statut==1 || $object->type==2)
		{
			...
			if($res < 0){
				...
				mkdir(DOL_DATA_ROOT.'/facture/'.$num, 0777);
				//dol_syslog('DIRECTORIO:'.DOL_DATA_ROOT.'/facture/'.$num);
		...
		else 
		{
			...
			else 
			{
				/* if($data['payment_type']=='6' && $aux2=='2'){
					$data2['ref']=$data['ref'];
					$data2['id']=$data['id'];
					$payment=self::addPaymentFac($data2);
				} */
				//Adding Payments
	...
	Private function CreateFacture2($aryTicket)
	{
		global $db,$user,$conf;
					...
					foreach($payments as $payment)
					{
						...
						if($arsepa[0]==$auxnom && ($arsepa[1]!=0 && $arsepa[1]!=NULL && $arsepa[1]!='')){
							if($i==0){
								...
								${$i."data"}['id']=$data['id'];
								dol_syslog('PAGO3.2 DIFF::'.${$i."data"}['difpayment']);
								$payment=self::addPaymentFac(${$i."data"});
								$i++;
							}
						}
					}
			...
			foreach ( $lines as $line )
			{
				if(sizeof($line)>0)
				{
					if ($line['idProduct']>0)
		    		{
						...
						$product_static->fetch($line['idProduct']);
						...
						else 
						{
							$qty=$line['cant']*-1;
                                                ...
                                                // Esta linea se ha modificado para resolver el problema de descuentos 
//						$line['discount'] = $line['discount']+ $object->remise_percent;
						$line['description']= $line['description']." ".$line['note'];
						$cadena="id $idTicket, description $line[description] Precio $line[price] Cantidad $qty, IVA $line[tva_tx], tax1 $line[localtax1_tx], tax2 $line[localtax2_tx], idp $line[idProduct], descuento $line[discount], nota $line[note], fk_product_type $line[fk_product_type], preciottc $line[price_ttc], typo $line[price_base_type]";
						$res=$object->addline($idTicket, $line['description'].$cadena, $line["price"], $qty, $line['tva_tx'], $line['localtax1_tx'], $line['localtax2_tx'], $line['idProduct'], $line['discount'], $line['note'], $line['fk_product_type'], $line['price_ttc'], "HT");							
		    		}
				}
				else 
				{
    ...
    private function addFactureLines($lines, $idTicket,$isreturn=false)
    {
    		...
    		foreach ( $lines as $line )
    		{
    			if(sizeof($line)>0)
    			{
    				if ($line['idProduct']>0)
    				{
    					...
    					dol_syslog('STOCK REEL::'.$product_static->stock_reel);
    					dol_syslog('LINE CANT'.$line['cant']);
    					...
    					$line['discount'] = $line['discount']+ $object->remise_percent;
    					$line['label']= $line['label']." ".$line['note'];
    					$cadena="Label: $line[label], Precio: $line[price], Cantidad: $qty, IVA: $line[tva_tx], Localtax1: $line[localtax1_tx], Localtax2_tx: $line[localtax2_tx], id_producto: $line[idProduct], descuento: $line[discount], TipoPrecio: $line[price_base_type], PrecioTTC: $line[price_ttc], ProductType: $line[fk_product_type]";
    					dol_syslog('RES CADENA:: '.$cadena);
    					//$res=$object->addline($line["label"], $line["price"], $qty, $line["tva_tx"], $line["localtax1_tx"], $line["localtax2_tx"], $line["idProduct"], $line["discount"], '','',0,0,'', $line["price_base_type"], $line["price_ttc"], $line["fk_product_type"]);
    					$res=$object->addline($line["label"], $line["price"], $qty, $line["tva_tx"],'','',$line["idProduct"],$line["discount"],'','',0,0,'',$line["price_base_type"],$line["price_ttc"],$line["fk_product_type"]);
    				}
    			}
    			else
    			{
    	...
    	dol_syslog('ADDFactureLines::'.$res);
    	return $res;
	...
	public static function getHistoric($ticketnumber='',$stat, $terminal='',$seller='',$client='',$amount='',$months=0,$years=0)
	{
		...
		$sql.= ' u.firstname, u.lastname,';//AMM 1 20140327 cambio u.name por u.lastname
		...
		$sql.= ' WHERE f.fk_soc = s.rowid AND t.rowid='.$_SESSION['TERMINAL_ID'];
		...
		if ($seller)
		{
			...
			$sql.= ' OR u.lastname LIKE \'%'.$db->escape(trim($seller)).'%\')';//AMM 1 20140327 cambio u.name por u.lastname
		}
		if ($client)
		{
	...
	public static function countHistoric()
	{
		...
		$sql.= ' WHERE f.entity = '.$conf->entity.' AND fk_cash='.$_SESSION['TERMINAL_ID'];
		...
		dol_syslog('ESTE QUERY CUENTA: '.$sql);
		$res = $db->query($sql);
	...
	private function addPayment($aryTicket)
	{
        ...
        {	/*MV*/
        	if ($aryTicket['payment_type']!=$cash->fk_modepaybank)
        	{
        		$sqlv="SELECT fk_bankacount
					FROM ".MAIN_DB_PREFIX."pos_terminal_mpagos
					WHERE fk_terminal=".$_SESSION['TERMINAL_ID']." AND fk_modepay=".$aryTicket['payment_type'];
        		$res=$db->query($sqlv);
        		$rqs=$db->fetch_object($res);
        		$bankaccountid = $rqs->fk_bankacount ;
        		$amount = $aryTicket['customerpay'];
        	}else{
        		$sqlv="SELECT fk_paybank FROM ".MAIN_DB_PREFIX."pos_cash WHERE rowid=".$_SESSION['TERMINAL_ID'];
        		$res=$db->query($sqlv);
        		$rqs=$db->fetch_object($res);
        		$bankaccountid = $rqs->fk_paybank ;
		...
		$payment->num_paiement='';
dol_syslog('ESTE BANKID::'.$bankaccountid);
	...
	private function addPaymentFac($aryTicket)
	{
		...
		$terminal = $_SESSION['TERMINAL_ID'];
		$cash->fetch($terminal);
		 dol_syslog('ESTE TERMINAL CASH::'.$terminal);
		if ($aryTicket['payment_type']==$cash->fk_modepaycash)
		{
			$bankaccountid = $cash->fk_paycash;
		...
		//dol_syslog('ESTE TERMINAL CASH BANK::'.$bankaccountid);
		//A�adir el posible pago de puntos
		...
		$paiement_id = $payment->create($userstatic);
		//dol_syslog('ESTE Payment create::'.$paiement_id.'::'.$payment->bank_account);
		if ($paiement_id > 0)
		{
			$result=$payment->addPaymentToBank($user,'payment','(CustomerFacturePayment)',$bankaccountid,$aryTicket['customerId'],'','');
			//dol_syslog("ESTE RESULT PAYMENT::".$result);
			if (! $result > 0)
			{
				return -1;
			}
	...
	{	
		...
		$object->tva_assuj = 1;
        $object->status= 1;
        $sql="SELECT CONVERT(value, SIGNED) as a FROM ".MAIN_DB_PREFIX."const WHERE name='MAIN_INFO_SOCIETE_COUNTRY' AND entity='".$conf->entity."'";
		$ares=$db->query($sql);
		$bres=$db->fetch_row($ares);
        $object->country_id=$bres['0'];
	...
	{	
		...
		$code_pays="'".$mysoc->country_code."'";//$code_pays="'".$mysoc->pays_code."'";
		...
		$myproduct->price_ttc          	= str_replace(',', '',number_format($aryProduct['price_ttc'],2));
	...
	public static function fillMailTicketBody($id)
	{
		...
		$message = $conf->global->MAIN_INFO_SOCIETE_NOM." ".$conf->global->MAIN_INFO_SOCIETE_ADRESSE." \n". $conf->global->MAIN_INFO_SOCIETE_CP.' '.$conf->global->MAIN_INFO_SOCIETE_VILLE." \n \n";
	...
	public static function permisoDescuento()
	{
		global $db;
		$sql="SELECT permiso FROM ".MAIN_DB_PREFIX."pos_cashgroup_cash WHERE fk_user='".$_SESSION['uid']."'";
		$res=$db->query($sql);
		$res1=$db->fetch_row($res);
		$aux=$res1['0'];
		return $aux;
	}
	public static function calculePrice($product)
	{
		...
		$info_bits = 0;
                // Las lineas acontinuaci�n han sido modificadas para resolver el problema de desceuntos acumulados
		$remise_percent_ligne;
		$remise_percent_global;
//                if($remise_percent_global != 0){
//                    $pu = $pu - ( $pu * ($remise_percent_global / 100) );
//                }
	...
	function getWarehouse($status=1)
	{
		global $db;
		$liste = array();
        $sqla='SELECT fk_warehouse FROM '.MAIN_DB_PREFIX.'pos_cash WHERE rowid='.$_SESSION['TERMINAL_ID'];
        $resulta = $db->query($sqla);
        $rowa = $db->fetch_row($resulta);
		$sql = "SELECT rowid, lieu";
		$sql.= " FROM ".MAIN_DB_PREFIX."entrepot";
		$sql.= " WHERE entity IN (".getEntity('warehouse', 1).") AND rowid='".$rowa[0]."'";
	...
	function get_prod($idCat,$more)
	{
        ...
        // M�todo para validar que una cantidad de items no exceda la cantidad en el stock
        public static function validaCantidadStock($datos){
            global $db,$conf;
            $function = "validando stock";
            $sql="SELECT value as val FROM ".MAIN_DB_PREFIX."const WHERE name='POS_STOCK' AND entity='".$conf->entity."'";
            $resql = $db->query($sql);
            $r = $db->fetch_object($resql);
            dol_syslog('ESTE VALUE:: '.$sql);
            if($r->val==1){
            	$res["data"] = "true";
            	return $res;
            }
            $sql ="SELECT reel FROM ".MAIN_DB_PREFIX."product_stock WHERE fk_product = ".$datos["idProd"]." AND fk_entrepot = ".$datos["warehouse"]." ";
            dol_syslog('ESTE QUERY CUENTA CANTIDAD:: '.$sql);
            $resql = $db->query($sql);
            $stock = $db->fetch_object($resql);
            $res = array();
            if($datos["cant"] <= $stock->reel){
                $res["data"] = "true";
            }else{
                $res["data"] = "false";
            }
        ...
        // M�todo para validar la cantidad de items de un tipo correspondientes a un ticket que no allan sido debueltos a�n
        public static function validaCantidadTicket($datos){
            global $db,$conf;
            /*$sql =  "SELECT qty + (
                                    IF( (SELECT
                                                    SUM(qty)
                                            FROM
                                                    ...
                                                    llx_pos_ticket
                                            WHERE
                                                    fk_ticket_source = '".$datos["idTicket"]."'
                                            AND fk_ticket = llx_pos_ticket.rowid
                                            AND fk_product = '".$datos["idProduct"]."')
                                   IS NULL , 0 , 
                                            (SELECT
                                                    SUM(qty)
                                            FROM
                        ...
                        WHERE fk_product = '".$datos["idProduct"]."'
