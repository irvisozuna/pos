...
class Ticket extends CommonObject
{
    ...
    function create($user)
    {
        ...
        if ($resql)
        {
            ...
            {
            	if($this->type == 1){
            		$consulta="SELECT entity, fk_c_paiement, customerpay FROM ".MAIN_DB_PREFIX."pos_multi_payment_ticket
            				WHERE fk_ticket=".$this->id_source;
            		$resultado=$this->db->query($consulta);
            		$i=0;
            		if($resultado){
            			$num=$this->db->num_rows($resultado);
            			$i=0;
            			while($i<$num){
            				$objp=$this->db->fetch_object($resultado);
            				$datTicketM[$i]["entity"]=$objp->entity;
            				$datTicketM[$i]["fk_c_paiement"]=$objp->fk_c_paiement;
            				$datTicketM[$i]["customerpay"]=$objp->customerpay;
            				$i++;
            			}
            			$i=0;
            			while($i<$num){
	            			$consulta='INSERT INTO '.MAIN_DB_PREFIX."pos_multi_payment_ticket (fk_ticket, fk_c_paiement, entity, date, customerpay)
	    					values(".$this->id.",
	    							".$datTicketM[$i]["fk_c_paiement"].",
	    									".$datTicketM[$i]["entity"].",
	    											'".$this->db->idate($now)."',
	    													".($datTicketM[$i]["customerpay"]*(-1)).")";
            				$resultado=$this->db->query($consulta);
            				$i++;
            			}
            		}
            	}else{
	            	if(isset($this->cash_payment) && $this->cash_payment>0){
	            		
		            	$query="INSERT INTO ". MAIN_DB_PREFIX . "pos_multi_payment_ticket (fk_ticket, entity, date, customerpay, fk_c_paiement)";
		            	$query.="values (".$this->id. ",".$conf->entity.",'".$this->db->idate($now)."', ".$this->cash_payment.", 4)";
		            	$resquery=$this->db->query($query);
	            	}
	            	if(isset($this->card_payment) && $this->card_payment>0){
	            	
	            		$query="INSERT INTO ". MAIN_DB_PREFIX . "pos_multi_payment_ticket (fk_ticket, entity, date, customerpay, fk_c_paiement)";
	            		$query.="values (".$this->id. ",".$conf->entity.",'".$this->db->idate($now)."', ".$this->card_payment.", 6)";
	            		$resquery=$this->db->query($query);
            		}
            	}
				$this->db->commit();
     ...
     */
    function addline($facid, $desc, $pu_ht, $qty, $txtva, $txlocaltax1=0, $txlocaltax2=0, $fk_product=0, $remise_percent=0, $note='', $type=0, $pu_ttc=0, $price_base_type, $date_start='', $date_end='', $ventil=0, $info_bits=0, $fk_remise_except='', $rang=-1, $special_code=0, $origin='', $origin_id=0, $fk_parent_line=0)
    {
        ...
        {
            $pu=$pu_ht;
        	//$pu=price2num($pu_ttc/(1+($txtva/100)));
        }
        else
        {
            $pu=$pu_ttc;
       ...
       // {
            ...
            $descuento=0;
            if(isset($remise_percent) && $remise_percent>0){
            	$descuento=($pu*($remise_percent/100))*$qty;
            }
            /*
            $total_ht  = $tabprice[0];//$qty*$pu-$descuento;
            ...
            $pu_ht = $tabprice[3];
			*/
            $total_ht=($qty*$pu)-$descuento;
            $total_tva=($total_ht*($txtva/100));
            if($total_tva<0){
            	$total_tva=$total_tva*-1;
            }
            $total_ttc=$total_ht<0 ? $total_ht*-1+$total_tva : $total_ht+$total_tva;
            $total_localtax1=0;
            $total_localtax2=0;
            $pu_ht=$pu;
            // Rang to use
            ...
            //$cadena="TotalHT:$total_ht TotalTVA $total_tva TotalTTC $total_ttc Precio: $pu, Cantidad: $qty, IVA: $tva_tx, IMP1: $txlocaltax1, IMP2: $txlocaltax2, idprod: $fk_product, descuento: $remise, nota: $note, , preciottc: $total_ttc";
            //$desc=$cadena;
            $this->line->desc=$desc;
            ...
            if ($result > 0)
            {
                ...
                if(!empty($facid) && $facid>0){
	                $sql="update ". MAIN_DB_PREFIX. "pos_ticket set
                            tva=ROUND((select sum(total_tva) from ".MAIN_DB_PREFIX."pos_ticketdet where fk_ticket=$facid),2), 
                            total_ht=ROUND((select sum(total_ht) from ".MAIN_DB_PREFIX."pos_ticketdet where fk_ticket=$facid),2),
                            total_ttc=ROUND((select sum(total_ttc) from ".MAIN_DB_PREFIX."pos_ticketdet where fk_ticket=$facid ) - (IF(remise_percent != 0 , ((select sum(total_ttc) from ".MAIN_DB_PREFIX."pos_ticketdet where fk_ticket=$facid ) * ( remise_percent / 100 ) ) , 0 ) ) ,2 )
                            where rowid=$facid";
                            $resql=$this->db->query($sql);
	                $resql=$this->db->query($sql);
                }
   ...
   /**
    ...
    function create_facture()
    {
    	...
    	for ($i = 0 ; $i < sizeof($this->lines) ; $i++)
    	{
    		...
    		//dol_syslog('AQUI ESTA ESTO: '.$this->lines[$i]->desc);
    	...
    	else
    	{
    		...
    		$sql = 'SELECT entity, fk_c_paiement, customerpay from ' . MAIN_DB_PREFIX . "pos_multi_payment_ticket WHERE fk_ticket=".$this->id;
    		$resql=$this->db->query($sql);
    		if($resql){
    			$num=$db->num_rows($resql);
    			$i=0;
    			...
    			$i=0;
    			while($i<$num){
    				echo $sql='INSERT INTO '.MAIN_DB_PREFIX."pos_multi_payment_facture (fk_facture, fk_c_paiement, entity, date, customerpay)
    					values(".$facture->id.", 
    							".$datTicketM[$i]["fk_c_paiement"].", 
    									".$datTicketM[$i]["entity"].", 
    											'".$this->db->idate($now)."', 
    													".$datTicketM[$i]["customerpay"].")";
    				$resql=$this->db->query($sql);
    				$i++;
    			}
    		}else{
