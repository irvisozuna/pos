...
class Ticket extends CommonObject
{
    ...
    function addline($facid, $desc, $pu_ht, $qty, $txtva, $txlocaltax1=0, $txlocaltax2=0, $fk_product=0, $remise_percent=0, $note='', $type=0, $pu_ttc=0, $price_base_type, $date_start='', $date_end='', $ventil=0, $info_bits=0, $fk_remise_except='', $rang=-1, $special_code=0, $origin='', $origin_id=0, $fk_parent_line=0)
    {
     ...
     //       	$descuento=$pu*($remise_percent/100);
			...
			*/
            ...
            // Insert line
            $sql = "SELECT type FROM ". MAIN_DB_PREFIX. "pos_ticket WHERE rowid = ".$facid." ";
            $result_set=$this->db->query($sql);
            $tipo = $result_set->fetch_assoc();
             if($tipo["type"] == 1){
                 $this->line->remise_percent=0;
                 $this->line->remise=0;
             }else{
            ...
            $result=$this->line->insert();  // Regresa la fila en la que se agrega el registro
            if ($result > 0)
            {
                ...
                if($tipo["type"] == 1){
                    $sql="update ". MAIN_DB_PREFIX. "pos_ticket set
                            ...
                            total_ttc=ROUND(((select sum(total_ttc) from ".MAIN_DB_PREFIX."pos_ticketdet where fk_ticket=$facid ),2)
                            where rowid=$facid";
                            $resql=$this->db->query($sql);
                }else{
...
class TicketLigne extends CommonObject
{
    ...
    function update()
    {
        ...
        $sql.= ",subprice=ROUND(".price2num($this->subprice).",2)";
        $sql.= ",price=ROUND(".price2num($this->price).",2)";
        ...
        {
        	$sql.= ",total_ht=ROUND(".price2num($this->total_ht).",2)";
        	$sql.= ",total_tva=ROUND(".price2num($this->total_tva).",2)";
        	$sql.= ",total_ttc=ROUND(".price2num($this->total_ttc).",2)";
        }
