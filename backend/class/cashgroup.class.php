 ...
 */
class CashGroup extends CommonObject
{
	...
	function fetch($id='', $groupname='')
	{
		...
		$sql = "SELECT g.rowid, g.entity, g.name ";
		$sql.= " ,'' AS note"; //$sql.= " , g.note";
		$sql.= " , g.datec";
		$sql.= " , '' AS datem";//$sql.= " , g.tms as datem";
		$sql.= " FROM ".MAIN_DB_PREFIX."pos_cash as g";
		if ($groupname)
		{
			$sql.= " WHERE g.name = '".$this->db->escape($groupname)."'";
		}
		else
		{
	...
	function listGroupsForUser($userid)
	{
		...
		$sql = "SELECT t.rowid, ut.entity as usergroup_entity";
		$sql.= " FROM ".MAIN_DB_PREFIX."pos_cash as t,";
		$sql.= " ".MAIN_DB_PREFIX."pos_cashgroup_cash as ut";
		$sql.= " WHERE ut.fk_cashgroup = t.rowid";
		$sql.= " AND ut.fk_user = ".$userid;
		if(! empty($conf->multicompany->enabled) && $conf->entity == 1 && $user->admin && ! $user->entity)
		{
			$sql.= " AND t.entity IS NOT NULL";
		}
		else
		{
			$sql.= " AND t.entity IN (0,".$conf->entity.")";
		}
		$sql.= " ORDER BY t.name";
		...
		if ($result)
		{
			while ($obj = $this->db->fetch_object($result))
			{
				...
				{
					$newgroup=new CashGroup($this->db);
	 ...
	 *  @return int  				<0 if KO, >0 if OK
	 */
	function SetInGroup($group, $entity, $id, $notrigger=0)
	{
		...
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."pos_cashgroup_cash";
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."pos_cashgroup_cash (entity, fk_user, fk_cashgroup)";
		$sql.= " VALUES (".$entity.",".$id.",".$group.")";
	 ...
	 *  @return int  			     <0 if KO, >0 if OK
	 */
	function RemoveFromGroup($group, $entity, $id, $notrigger=0)
	{
		global $conf,$langs,$user;
