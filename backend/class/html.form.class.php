...
class FormCash
{
    ...
    function select_dolusers($selected='', $htmlname='userid', $show_empty=0, $exclude='', $disabled=0, $include='', $enableonly='', $force_entity=0, $maxlength=0, $showstatus=0)
    {
        ...
        if (! empty($user->socid)) $sql.= " AND u.fk_societe = ".$user->socid;
    ...
    {
        print $this->formconfirm2($page, $title, $question, $action, $formquestion, $selectedchoice, $useajax, $height, $width);
    }
     ...
     */
    function formconfirm2($page, $title, $question, $action, $formquestion='', $selectedchoice="", $useajax=0, $height=170, $width=500)
    {
        global $langs,$conf;
    ...
    function showphoto($modulepart,$object,$width=100)
    {
        ...
        else if ($modulepart=='userphoto')
        {
            ...
            if ($object->photo) $file=get_exdir($id, 2, 0, 0, $object, 'user').getImageFileNameForSize($object->photo, '_mini');
    ...
    function select_dolgroups($selected='', $htmlname='groupid', $show_empty=0, $exclude='', $disabled=0, $include='', $enableonly='', $force_entity=0)
    {
        ...
        $sql = "SELECT ug.rowid, ug.name as nom";
        if (! empty($conf->multicompany->enabled) && $conf->entity == 1 && $user->admin && ! $user->entity)
        {
            $sql.= ", e.label";
        }
        $sql.= " FROM ".MAIN_DB_PREFIX."pos_cash as ug ";
        if (! empty($conf->multicompany->enabled) && $conf->entity == 1 && $user->admin && ! $user->entity)
        {
