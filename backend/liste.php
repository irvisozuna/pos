...
if (!$user->rights->pos->backend)
{
	...
	if ($_GET['search_user'])
	{
		...
		$sql.= ' OR u.lastname LIKE \'%'.$db->escape(trim($_GET['search_user'])).'%\')';//AMM 1 20140327 cambio u.name por u.lastname
	}
	...
	if ($resql)
	{
		...
		print_liste_field_titre($langs->trans('User'),$_SERVER['PHP_SELF'],'u.lastname','',$param,'align="center"',$sortfield,$sortorder);//AMM 1 20140327 cambio u.name por u.lastname
