...
dol_fiche_head($head, 'ticket', $langs->trans("Ticket"), 0, 'company');
...
if ($result > 0)
{
	...
	*/
	...
	//Local taxes
	if ($mysoc->country_code=='ES')//if ($mysoc->pays_code=='ES')
	{
	...
	if ($mysoc->country_code=='ES')//if ($mysoc->pays_code=='ES')
	{
		...
		{
			print '<tr><td>'.$langs->transcountry("AmountLT1",$mysoc->country_code).'</td>';//print '<tr><td>'.$langs->transcountry("AmountLT1",$mysoc->pays_code).'</td>';
			print '<td align="right" colspan="2" nowrap>'.price($object->total_localtax1).'</td>';
			print '<td>'.$langs->trans(currency_name($conf->currency)).'</td></tr>';
		}
		if ($mysoc->localtax2_assuj=="1") //Localtax2 IRPF
		{
			print '<tr><td>'.$langs->transcountry("AmountLT2",$mysoc->country_code).'</td>';//print '<tr><td>'.$langs->transcountry("AmountLT2",$mysoc->pays_code).'</td>';
	...
	*/
	...
	{
		$ret=$form->formconfirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('DeleteTicket'), $langs->trans('ConfirmDeleteTicket',$object->ticketnumber), 'confirm_deleteticket','','',1);
		print @$ret;
		//if ($ret == 'html') print '<br>';	
	}
	if (GETPOST('action','string') == 'abandon')
	{
		$ret=$form->formconfirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('ClassifyCanceled'), $langs->trans('ConfirmAbandonTicket',$object->ticketnumber), 'confirm_abandonticket','','',1);
		print @$ret;
		//if ($ret == 'html') print '<br>';	
	}
	if (GETPOST('action','string') == 'facture')
	{
		$ret=$form->formconfirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans('CreateBill'), $langs->trans('ConfirmCreateBill',$object->ticketnumber), 'confirm_factureticket','','',1);
		print @$ret;
		//if ($ret == 'html') print '<br>';	
