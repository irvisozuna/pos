 ...
 */
function check_user_password_dolibarr($usertotest,$passwordtotest,$terminaltest,$entitytotest=1)
{
...
// 		$sql.=' AND '.$entitycol." IN (0," . ($entity ? $entity : 1) . ")";
		...
		$sql = "SELECT u.rowid, u.entity, u.pass, u.pass_crypted";//AMM 1 20140327 cambio u.name por u.lastname
		$sql.= " FROM ".MAIN_DB_PREFIX."user u";
		$sql.= " INNER JOIN ".MAIN_DB_PREFIX."pos_cashgroup_cash cg ON u.rowid = cg.fk_user";
		$sql.= " INNER JOIN ".MAIN_DB_PREFIX."pos_cash c ON cg.fk_cashgroup = c.rowid";
		$sql.= " WHERE u.login = '".$db->escape($usertotest)."'";
		$sql.= " AND c.rowid = ".$db->escape($terminaltest);
		$sql.= " AND u.entity IN (0," . ($entity ? $entity : 1) .")";
        //print_r($sql); die();
