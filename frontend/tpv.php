...
<div class="header darkblue gradient"  >
   ...
   <!--<a class="top_help but" href="http://2byte.gotdns.com/liveagent/index.php?type=page&urlcode=715850&title=M%C3%B3dulo-DoliPOS&r=1" title="<?php echo $langs->trans("OnlineHelp"); ?>" target="_new"></a>-->
    ...
    <a class="top_infoproduct" id="id_btn_infoproduct" href="#" title="<?php echo $langs->trans("InfoProduct"); ?>"></a> -->
 	<?php if($usertpv->rights->pos->changeuser){?>
...
<p id="top_sep"> <br clear="all" />  </p>
    ...
    <div class="ticket_content ui-layout-center ui-widget-content add-scrollbar ui-layout-pane ui-layout-pane-center ui-layout-pane-hover ui-layout-pane-center-hover ui-layout-pane-open-hover ui-layout-pane-center-open-hover" style="">
           ...
           <!-- FIN berni -->
			...
			<div class="options">
				<ul>
					...
					<br clear="all" />
					<?php 
					$sqla="SELECT permiso FROM ".MAIN_DB_PREFIX."pos_cashgroup_cash WHERE fk_user=".$_SESSION['uid'];
					...
					?>
					<input onclick="this.select()" type="hidden" size="5" maxlength="3" name="line_discount" id="line_discount"  value="0" class="numKeyboard"></li>
					<?php
					}
					?>
			...
			<?php 	
				...
				echo "<div class='payment_types'><a class='btn3dbig' id='paytypeall' style='height:40px;'>Mixto</a></div>";
			?>	
			</div>
			</div>
			...
			<div id="payment_options">
				<div id="payment_camp" class="payment_options">
					...
					<label><?php echo $langs->trans("CustomerPay");?></label>
						<input style="height:35px; font-size:30px;" onclick="this.select()" type="text" value="" name="pay_client_id" id="pay_client_id" class="numKeyboard" required>
					</div>
					<label><?php echo $langs->trans("CustomerRet");?></label>	
					<div class="payment_return"></div>
				</div>
				<!-- MV -->
				<div id="payment_campall" class="payment_options">
					<div id="payment_total_points">
					<label><?php echo $langs->trans("Total");?></label>
					<div  class="payment_total"></div>
					</div>
					...
					if(sizeof($payments))
					{
						foreach($payments as $payment)
						{
							echo "<label>".$payment['label']."</label><input style='width:50%;height:35px; font-size:30px;' onclick='this.select()' type='text' value='' id='pay_client_id".$payment['id']."' name='pay_client_id".$payment['id']."' class='numKeyboard'>";
						}
					}
					?>
					</div>
		...
		<div id="idFactureMode" class="leftBlock bloqueOpciones" style="display:none" title="<?php echo $langs->trans("Facture"); ?>">
			<div class="options">
				<div>
					...
					<!-- Se quita el boton de factura simplificada ya que en mexico no se ocupa -->
			...
			<table id="customerTable" class="tableList">
				<thead>
					<tr>
						...
						<th><?php echo $langs->transcountry('ProfId1',$mysoc->country_code);//echo $langs->transcountry('ProfId1',$mysoc->pays_code); ?></th>
       ...
       <div id="ticketRight"> 
			...
			<?php 
			$sql="SELECT pcproducto
					FROM ".MAIN_DB_PREFIX."pos_cashgroup_cash
					WHERE fk_user=".$_SESSION["uid"];
			$rs=$db->query($sql);
		...
		<div id="idClient" class="bloqueOpciones" style="display:none" title="<?php echo $langs->trans("AddCustomer"); ?>">
			<div class="options">
				<ul>
                    ...
                    <br clear="all" />
					<li><label><?php echo $langs->transcountry('ProfId1',$mysoc->country_code);//echo $langs->transcountry('ProfId1',$mysoc->pays_code);?>:</label><input onclick="this.select()" type="text" name="id_customer_cif" id="id_customer_cif" class=""></li>
					<br clear="all" />
		...
		<div id="idChangeCustomer" class="bloqueOpciones" style="display:none; height:400px !important;" title="<?php echo $langs->trans("ChangeCustomer");?>">
			...
			<table id="customerTable_" class="tableList" style="float:left;">
				<thead>
					<tr>
						<th style="display:none"><?php echo $langs->trans("ID"); ?></th>
						<th width=100px; style="color: #fff;"><?php echo $langs->transcountry('ProfId1',$mysoc->country_code);//echo $langs->transcountry('ProfId1',$mysoc->pays_code); ?></th>
						<th><?php echo $langs->trans("Name"); ?></th>
