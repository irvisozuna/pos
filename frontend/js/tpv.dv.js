...
var Ticket = jQuery.Class({
	...
	checkExistReturnProduct:function(idProduct){
		if(typeof _TPV.ticket.oldproducts!='undefined' && _TPV.ticket.oldproducts.length>0)
		{
			...
			{
				if(_TPV.ticket.oldproducts[i]['idProduct']==idProduct){
                                    // Esta condición se agrega para reslver el problema en que se permite devlver más productos de los que se tienen en un ticket
                                    var datos = new Object();
                                    datos.idProd = idProduct;
                                    for( x in _TPV.ticket.oldproducts){
                                        if(_TPV.ticket.oldproducts[x].idProduct == idProduct){
                                            datos.cantProduct = _TPV.ticket.oldproducts[x].cant;
                                        }
                                    }
	...
	newTicket: function(){
		...
		this.discount_percent = _TPV.discount;
                // Esta condicion se agrega para correjir el error de devoluciones de productos con stock 0
	...
	calculeTotal : function(){
			...
			if(parseFloat(line["remise_percent_global"]) != parseFloat(discount))
				{
					...
					if(!line["price_base_type"]) line["price_base_type"] = "TTC";                                        
                                        if(this.type == 1){
                                            line["discount"] = 0;
					...
					this.lines[i].total = result["total_ttc"];
                                        if(this.type == 1)  this.lines[i].total = this.lines[i].total - (this.lines[i].total * (this.discount_percent/100) );
		...
		pricediscount = Math.round(pricediscount*100)/100;
                // Esta linea se agrega para resolver el problema de los decuentos multiples
                if(this.discount_percent != 0){
                    pricediscount += (this.total / ( (100 - this.discount_percent) / 100 ) ) - this.total ;
                    if(this.type == 1)  this.total = this.total - (this.total * (this.discount_percent/100) );
                } 
		$('#totalDiscount').html(displayPrice(pricediscount));
		var total = new Number(this.total);
		total = total.toFixed(2);
	...
	editTicketLine : function(idProduct)
	{
		...
		$('#id_btn_editTicketline').click(function(){
                    console.log("modificando linea")
                    console.dir(_TPV)
			if(_TPV.ticket.checkApplyQuantity(idProduct,$('#line_quantity').val()) || _TPV.ticketState == 2 )
			{
		...
		if(_TPV.ticket.type==1)
		{
			...
			$('#id_btn_ticketRet').click(function(){
				$('#id_btn_ticketRet').unbind('click');
                                // Esta condición se agrega para correjir el error de los iva's negativos cuando se hace una devolución
//                                if(_TPV.ticketState == 2){
//                                    for( x in _TPV.ticket.lines){
//                                        _TPV.ticket.lines[x].tva_tx = "-"+_TPV.ticket.lines[x].tva_tx;
//                                    }
//                                } 
				_TPV.ticket.mode=0;
