...
// Esta opción es creada para resolver el problema de devoluciones con stock =< 0
elseif($action=='searchProducts2')
{
	if(sizeof($data))
	{
		$searchValue = $data['data']['search'];
		$warehouse = $data['data']['warehouse'];
		$result = POS::SearchProduct2($searchValue, false, $warehouse,1);
		echo json_encode($result);
...
elseif($action=='PermisoDescuento')
{
	$result = POS::permisoDescuento();
    echo json_encode($result);
}
...
// Esta condicion se agrega para corregir el problema de ventas que superan el stock
elseif($action == "validaCantidad"){
    $result = POS::validaCantidadStock($data["data"]);
    echo json_encode($result);
}
// ESta condiciión se agrega para corregir el problema de esceso de items en devolución
elseif($action == "validaDevol"){
