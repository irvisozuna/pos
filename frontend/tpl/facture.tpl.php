		...
		if ($result)
		{
			...
			if (! empty($object->lines))
			{
				...
				foreach ($object->lines as $line)
				{
					...
					echo ('<tr><td align="left">'.$line->description.'</td><td align="right">'.$line->qty." * ".number_format($line->subprice,2).'</td><td align="right">'.$line->remise_percent.'%</td><td class="total">'.number_format($line->total_ht,2).' '.$langs->trans(currency_name($conf->currency)).'</td></tr>'."\n");
					$subtotal+=$totalline;
				}
			}
			else 
...
<table class="totaux">
	<?php
		...
		$sqlv="SELECT b.fk_paiement,a.amount,c.code,c.id
			FROM ".MAIN_DB_PREFIX."paiement_facture a, ".MAIN_DB_PREFIX."paiement b, ".MAIN_DB_PREFIX."c_paiement c
			WHERE fk_facture=".$id." AND a.fk_paiement=b.rowid 
			AND b.fk_paiement=c.id";
		$rqsv=$db->query($sqlv);
		$nrsv=$db->num_rows($rqsv);
		if($object->type==0 && $nrsv>1){
			$pay2=0;
